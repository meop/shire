...
- **Shell-agnostic abstractions**: Write once, deploy shell-specific scripts to different shells
...
class MyCmd extends CmdBase {
  ...
  override async work(
    ...
    return shell
      .with(shell.varSetStr(['TARGET'], target))
      .with(await shell.fileLoad(['mycommand']))
      .with(['runMyCommand'])
      .build()
  }
}
...
Deno.serve(async (request: Request) => {
  ...
  const shellType = parts[0] || 'zsh'
  const shell = shellType === 'nu' ? new Nushell() : shellType === 'pwsh' ? new Powershell() : new Zshell()
...
Create files in `sh/{shell}/` directories:
...
- **sh.ts** - Shell abstraction for shell implementations (`Nushell`, `Powershell`, `Zshell`)
...
Write shell-agnostic code using the `Sh` interface. Methods like `varSet()`, `print()`, and `fileLoad()` automatically
...
```typescript
shell.varSetStr(['KEY'], value) // single string value
shell.varSetArr(['KEYS'], values) // array of raw strings
```
